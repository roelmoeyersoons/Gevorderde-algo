...
private:
	...
	bool repOKRec(const Zoekboom<Sleutel,Data> &s, bool links) const;
};
...
bool Zoekboom<Sleutel,Data>::repOK() const{
	...
	return (*this)->links.repOKRec((*this), true) 
				&& (*this)->rechts.repOKRec((*this), false);
	
}

template <class Sleutel,class Data>
bool Zoekboom<Sleutel,Data>::repOKRec(const Zoekboom<Sleutel,Data> &z, bool links) const{
	if(*this == nullptr){
		return true;
	}
	bool binboomOK = links? (*this)->sleutel < z->sleutel : (*this)->sleutel > z->sleutel;
	//std::cout << ((*this)->ouder == z.get());
	if (!binboomOK || (*this)->ouder != z.get())
		return false;
	return (*this)->links.repOKRec((*this), true) 
				&& (*this)->rechts.repOKRec((*this), false);
